...
void setup()
{
  ...
  img2d = createImage(600,600, RGB);
  pg2d = createGraphics(600,600);
  pg3d = createGraphics(600,600,OPENGL);
  pg3d.beginCamera();
  pg3d.camera(300,300,200,0,0,0,0,1,0);
  pg3d.endCamera();
  
  pg3d.beginDraw();
  pg3d.background(0,0,0);
  pg3d.endDraw();
...
void draw()
{
    ...
    //filter(DILATE);
    fastblur(pg2d.pixels, 25);

    pg2d.updatePixels();
    
    pg2d.filter(DILATE);
    pg2d.filter(POSTERIZE, 2);
    pg2d.filter(ERODE);
    pg2d.filter(ERODE);
    //filter(ERODE);
    //filter(ERODE);
    ...
    pg3d.beginCamera();
    thetac += 2*PI/300;
    pg3d.camera(1200,1200,1200,0,0,-3*width,0,0,-1);
    pg3d.endCamera();
    
    pg3d.beginDraw();
    //pg3d.background(0,0,0);
    pg3d.strokeWeight(0);
    //pg3d.pointLight(100,100,100, 400,400,400);
    pg3d.pointLight(255,255,255, 800,800,800);
    img2d.loadPixels();
    for(int i = 0; i < 100; i++) {
      for (int j = 0; j < 100; j++) {
        int ct = i*100+j;
        color pxCol = img2d.pixels[ct];
        if (red(pxCol) > 0) {
          //println(red(pxCol) + "_" + i + "_" + j);
          pg3d.pushMatrix();
          pg3d.translate(width*.5+10*j, height*.5+10*i, -1000+5*ctr3d);
          ctrclr = 255-(2.0*ctr3d/3.0+55);
          pg3d.fill(ctrclr,ctrclr,ctrclr,30);
          pg3d.stroke(ctrclr);
          pg3d.box(10);
          pg3d.popMatrix();
        }
    ...
    /**for(int x=0; x<600; x++) {
      for(int y=0; y<600; y++) {
        ...
        int ym = 10*(y%10);
        println(y + " " + ym);
        pixels[x*600+y] = pixels[xm*600+ym]; 
      }
    }**/
    //fastblur(pixels, 25);
    //updatePixels();
...
void plot3D(PGraphics pg, PImage im) {
  pg.background(0);
  im.loadPixels();
  for(int i = 0; i < 100; i++) {
    for (int j = 0; j < 100; j++) {
      int ct = i*100+j;
      color pxCol = im.pixels[ct];
      if (red(pxCol) > 0) {
        pushMatrix();
        pg.translate(i*5, j*5, 0);
        pg.box(5);
        popMatrix();
      }
    }
...
void drawtick(PGraphics pg, int vw) {
  int c = 0;
  for(int i = sample; i < npts; i++) {
    pg.strokeWeight(sw[i]);
    float cl = 255.0*(npts-c) + (c)*co[i];
    float cl_shift = 255-cl/npts/fadeFactor; 
    pg.stroke(color(cl_shift, cl_shift, cl_shift));
    pg.line(x1[i]-dx1[i],y1[i]-dy1[i],x2[i]-dx2[i],y2[i]-dy2[i]);
    pg.line(x1[i]+dx1[i],y1[i]+dy1[i],x2[i]+dx2[i],y2[i]+dy2[i]);
...
void drawtick2(PGraphics pg, int vw) {
  int c = 0;
  for(int i = sample; i < npts; i++) {
    pg.strokeWeight(csw[i]);
    float cl = 255.0*(npts-c) + (c)*cco[i];
    float cl_shift = 255-cl/npts/fadeFactor; 
    pg.stroke(color(cl_shift, cl_shift, cl_shift));
    pg.line(cx1[i]-cdx1[i],cy1[i]-cdy1[i],cx2[i]-cdx2[i],cy2[i]-cdy2[i]);
    pg.line(cx1[i]+cdx1[i],cy1[i]+cdy1[i],cx2[i]+cdx2[i],cy2[i]+cdy2[i]);
